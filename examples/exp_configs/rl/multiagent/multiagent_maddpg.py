...
V_TARGET = 40 # target velocity /speed
...
flow_params = dict(
    ...
    net=NetParams(
        ...
        additional_params={
            ...
            "horizontal_lanes": LANE_NUMBER,
            "vertical_lanes": LANE_NUMBER,
        },
    ),
...
obs_space_dict = dict(zip([0,1,2,3,4,5,6,7,8], [obs_space for i in range(0,9)]))
act_space_dict = dict(zip([0,1,2,3,4,5,6,7,8], [act_space for i in range(0,9)]))

def gen_policy(i: int):
    """Generate a policy in RLlib."""
    return MADDPGTFPolicy, obs_space, act_space, { "agent_id": i, "use_local_critic": True, "obs_space_dict": obs_space_dict, "act_space_dict": act_space_dict}

POLICY_GRAPHS = {}
for i in range(0,N_ROWS*N_COLUMNS):
    #Setup PG with one policy per agent
    POLICY_GRAPHS['center'+str(i)] = gen_policy(i)

def policy_mapping_fn(s):
    """Map a policy in RLlib."""
    return s


POLICIES_TO_TRAIN = ['center0','center1','center2','center3','center4',
